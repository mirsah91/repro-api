...
const SYSTEM_PROMPT = [
  ...
  '- When querying traces and the question contains function, file, or line information, combine them with a single $or over codeRefs.fn / codeRefs.file / codeRefs.line. For example:',
  '  {',
  '    "$and": [',
  '      { "sessionId": "<sessionId>" },',
  '      {',
  '        "$or": [',
  '          { "codeRefs.fn":   { "$regex": "<fn>",   "$options": "i" } },',
  '          { "codeRefs.file": { "$regex": "<file>", "$options": "i" } },',
  '          { "codeRefs.line": <lineNumber> }',
  '        ]',
  '      }',
  '    ]',
  '  }',
