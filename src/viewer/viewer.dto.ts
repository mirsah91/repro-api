...
export class UiInfoDto {
    ...
    @ApiProperty({ required: false, description: 'Additional UI metadata', type: Object, additionalProperties: true })
    @IsOptional() @IsObject()
...
export class RequestEvtDto {
    ...
    @ApiProperty({ required: false, type: Object, additionalProperties: true }) @IsOptional() @IsObject()
    headers?: Record<string, any>;

    @ApiProperty({ required: false, description: 'Normalized grouping key (e.g. "GET /items")' })
    @IsOptional() @IsString()
    key?: string;
...
export class DbChangeDto {
    ...
    @ApiProperty({ type: DbPkDto }) @ValidateNested() @Type(() => DbPkDto)
    pk!: DbPkDto;
    ...
    @ApiProperty({ enum: DbOp }) @IsEnum(DbOp)
    op!: DbOp;
...
export class ActionWithDetailsDto extends ActionBaseDto {
    @ApiProperty({ type: [RequestEvtDto] }) @IsArray() @ValidateNested({ each: true }) @Type(() => RequestEvtDto)
    requests!: RequestEvtDto[];

    @ApiProperty({ type: [DbChangeDto] }) @IsArray() @ValidateNested({ each: true }) @Type(() => DbChangeDto)
    db!: DbChangeDto[];
}
...
export class RrwebMetaDto {
    ...
    @ApiProperty({ nullable: true }) @IsOptional()
...
export class RespDiffChangeDto {
    @ApiProperty({ description: 'JSON path (dot or keyed for arrays)' }) @IsString()
    path!: string;

    @ApiProperty({ enum: RespChangeType }) @IsEnum(RespChangeType)
    type!: RespChangeType;
...
export class RespDiffEdgeDto {
    ...
    @ApiProperty({ type: RespDiffSummaryDto }) @ValidateNested() @Type(() => RespDiffSummaryDto)
    summary!: RespDiffSummaryDto;

    @ApiProperty({ type: [RespDiffChangeDto] })
    @IsArray() @ValidateNested({ each: true }) @Type(() => RespDiffChangeDto)
...
export class RespDiffGroupDto {
    @ApiProperty({ example: 'GET /items' }) @IsString()
    key!: string;

    @ApiProperty({ type: [RespDiffCallDto] })
    @IsArray() @ValidateNested({ each: true }) @Type(() => RespDiffCallDto)
    calls!: RespDiffCallDto[];

    @ApiProperty({ type: [RespDiffEdgeDto] })
    @IsArray() @ValidateNested({ each: true }) @Type(() => RespDiffEdgeDto)
...
export class SummaryResponseDto {
    ...
    @IsArray() @ValidateNested({ each: true }) @Type(() => ActionBaseDto)
...
export class ActionDetailsResponseDto {
    ...
    @IsArray() @ValidateNested({ each: true }) @Type(() => RequestEvtDto)
    requests!: RequestEvtDto[];

    @ApiProperty({ type: [DbChangeDto] })
    @IsArray() @ValidateNested({ each: true }) @Type(() => DbChangeDto)
    db!: DbChangeDto[];
}
...
export class FullResponseDto {
    ...
    @IsOptional() @ValidateNested() @Type(() => RrwebMetaDto)
    rrweb?: RrwebMetaDto;

    @ApiProperty({ type: [ActionWithDetailsDto] })
    @IsArray() @ValidateNested({ each: true }) @Type(() => ActionWithDetailsDto)
    actions!: ActionWithDetailsDto[];

    @ApiProperty({ required: false, type: [RespDiffGroupDto] })
    @IsOptional() @IsArray() @ValidateNested({ each: true }) @Type(() => RespDiffGroupDto)
